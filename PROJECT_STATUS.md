...
### 本文の要件整備（仕様 01/§3.3）
- 各章に【物理的直観】【数値の現実】【発展】ボックスを1個以上配置（ch11/ch12 へ追補）。
- 全章の演習に\textbf{解答指針}を巻末付録G として追加。
- 導出の行間を補填（ch2 の $T_\nu/T_\gamma$ エントロピー全段、ch10 の $\mu$ 部分積分→$j_\ell$ 全段、
  ch3 Saha 全段、ch8 の 8/15 閉包導出など）。`make book` で 0エラー・0未定義参照・約35ページ。

## 既知の制約・残課題（次フェーズへ引き継ぎ）
- **本文の分量**: 現状 PDF 約35ページ（全14章＋付録、導出・ボックス・演習解答は完備）。
  仕様の 200ページ目標には更なる説明文・例題・図の加筆余地（構成・式・図・数値は揃済み）。
