...
| 音響ピーク（目視, $n_k=400$） | 220/540/810 | ≈220/540/800 | ✅ |
| 宇宙年齢 | ~13.8 Gyr | 13.81 | ✅ |

$C_\ell^{TT}$ は正しい音響構造（第一/第二/第三ピーク $\ell\approx220/540/800$、
SWプラトー $\mathcal D_\ell\sim10^3\,\mu K^2$、Silk減衰尾）を再現する
（`figures/cl_tt_fiducial.png`）。ピーク振幅は標準値より一様に約 +20〜30% 高い
（偏光無視・$Y_p=0$・CLASS未較正の残差; §5.3 で詰める）。振幅規格化は IC が
$\mathcal R=1$ 正規化（$\Psi_{\rm ini}=-\tfrac23\mathcal R/(1+\tfrac4{15}f_\nu)$）
である事実から $\mathcal R_{\rm ini}=1$ として確定（`spectrum._R_ini`）。
pytest は §5.1–5.2 の単体テスト全20件が緑。

## 既知の制約・残課題（次フェーズへ引き継ぎ）
- **CLASS比較（`03` §5.3）**: 当環境に `classy` 未導入のため未実施。
  キャッシュCSV比較モードの参照データ生成が必要。減衰尾（$\ell\gtrsim600$）は
  偏光無視の系統差＋k標本化ノイズが残り、CLASS較正で詰める。
- **高 $\ell$ ノイズ**: 二次k格子は低k密・高k疎で、高 $\ell$ の振動を取り
  こぼす。`04` §3 の収束テスト（$n_k\in\{150,250,400\}$）で既定値を確定する。
