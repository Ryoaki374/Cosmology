...
\subsection{赤方偏移（導出契約 S1.3）}
光子はヌル測地線 $ds^2=0$ を進む。計量\eqref{eq:flrw}（$k=0$）で動径方向に進む光子は
$c\,dt=a\,dr$。4元運動量 $P^\mu=dx^\mu/d\lambda$ のヌル測地線方程式 $\mu=0$ 成分
\begin{equation}
  \frac{dP^0}{d\lambda}+\Gamma^0_{ij}P^iP^j=0,\qquad \Gamma^0_{ij}=\frac{a\dot a}{c^2}\delta_{ij}
\end{equation}
にヌル条件 $P^0=|\mathbf P|/c$（背景）を入れると $\dot P^0/P^0=-\dot a/a$、すなわち
固有運動量 $p\equiv|\mathbf P|\propto1/a$。光子のエネルギー $E=pc\propto1/a$、振動数
$\nu\propto1/a$ なので波長は $\lambda\propto a$。放出時 $a$、観測時 $a_0=1$ として
\begin{equation}
  1+z \equiv \frac{\lambda_{\rm obs}}{\lambda_{\rm emit}}=\frac{a_0}{a}=\frac{1}{a}.
\end{equation}
これが宇宙論的赤方偏移で、CMB は $z_*\approx\valzstar$ の最終散乱面から来た光である。
...
等密度は $a_{\rm eq}=\Omega_{r0}/\Omega_{m0}$、数値で $z_{\rm eq}\approx\valzeq$
（\texttt{values.json} 由来のマクロ \texttt{\textbackslash valzeq}）。
...
\begin{figure}[ht]\centering
  \includegraphics[width=0.78\linewidth]{bg_density_fractions.png}
  \caption{密度分率 $\Omega_i(x)$ の進化（\texttt{cmbcore}, NB1）。放射優勢から物質優勢、
  さらに $\Lambda$ 優勢への遷移と等密度 $z_{\rm eq}$（縦線）を示す。}
  \label{fig:bg}
\end{figure}
